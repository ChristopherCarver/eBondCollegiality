...
The basic sequence for inbound messaging handling is the supplier will initiate the communication by passing a post REST payload to your instance. Your instance will perform the transform of the payload and add the ticket number and reference (sys_id) to the transform record. ServiceNow will then return the payload with the updated fields back to the supplier as a response to the initial REST call.
...
Each section will inform you if what you are about to setup is a standard component or a custom component within the framework at the start of each section. 


**Sequence Diagram**
\
The sequence of events for outbound communication is slightly more complex. See below sequence diagram as it lays out the various components for outbound handling.  
```mermaid
sequenceDiagram
    participant T as Incident
    participant R as Relationship
    participant B as Business Rule
    participant A as Supplier Script Action
    participant C as Supplier Script Include
    participant U as Script Include
    participant P as REST Payload Pool
    participant E as ECC Queue
    participant H as Supplier Business Rule
    participant S as Supplier
    
    T-->>B: incident change
    B-->>A: generate aysnc event for each supplier
    A->>C: supplier captures event and processes
    C->>U: load supplier payload
    U->>P: record REST payload for supplier
    U-->>S: async REST call passing payload
    S-->>E: retrieve response
    E-->>H: handle supplier response
    H->>R: update relationship
    H->>P: update REST payload
```

**Internal ServiceNow Flowchart** 
\
In the diagram below, the custom components are highlighted in green.
